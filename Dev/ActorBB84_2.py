...
class Eve(Actor):
    def __init__(self, name, next_actor, intercept_rate=0.2):
        super().__init__(name)
        self.next_actor = next_actor
    ...
    async def handle_message(self, msg):
        ...
        if prng.random() > self.intercept_rate:
...
def calculate_metrics(reference_key, subject_key, num_input_qubits, sample_size=0.1):
    ...
    if k < n_sifted:
        indices = PRNG.sample(range(n_sifted), k)
...
async def run_simulation_instance(num_qubits=100, include_eve=False, verbose=True, 
                                  ...
                                  qber_sample_size=0.1, eve_intercept_rate=0.5):
    if verbose:
        print("="*60)
    ...
    if include_eve:
        eve = Eve("Eve", next_actor=detector, intercept_rate=eve_intercept_rate)
    ...
    raw_metrics = calculate_metrics(sift_alice, sift_bob, num_qubits, sample_size=qber_sample_size)
    ...
    final_metrics = calculate_metrics(sift_alice, final_key_bob, num_qubits, sample_size=qber_sample_size)
