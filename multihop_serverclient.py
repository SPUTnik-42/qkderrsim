...
# Ensure the ServerClientBB84 folder is importable
ROOT_DIR = os.path.dirname(os.path.abspath(__file__))
SC_DIR = os.path.join(ROOT_DIR, "ServerClientBB84")
if SC_DIR not in sys.path:
    sys.path.insert(0, SC_DIR)

import bb84_server_client as sc
...
def simulate_optimal_hops(
    ...
    if relay_counts is None:
        relay_counts = np.linspace(1, 15, 20, dtype=int).tolist() #[1, 2, 4, 9, 14] 
    ...
    print(f"   -> Simulating Server-Client BB84 ({proto}/{pa})...")
...
def simulate_range_extension_protocols(
    ...
    """Run range-extension sweeps for multiple EC protocols and return results.

    Returns: (distances, results_dict) where results_dict[protocol] = (direct_rates, relayed_rates)
    """
    ...
    for proto in protocols:
        ...
        wrapper = ProtocolWrapperServerClient(protocol=proto, pa_protocol=cfg.get('pa_protocol','toeplitz'), num_qubits=num_qubits,
        ...
        # Store the same finite direct_rates for each protocol to keep return shape consistent
        results[proto] = (finite_direct_rates.tolist(), relayed_rates)

    return scan_distances, results


def plot_range_extension_protocols(distances, results, relays, config):
    """Plot direct vs relayed curves for multiple EC protocols side-by-side."""
    ...
    # Plot relayed curves per protocol
    for i, proto in enumerate(protocols):
        _, relayed_rates = results[proto]
        color = colors(i % 10)
        relayed_arr = np.asarray(relayed_rates, dtype=float)
        plt.plot(distances, relayed_arr, linestyle=linestyles['relayed'], color=color, linewidth=2, label=f'{proto} (relayed)')
    ...
    p_name = config.get('protocol','polar').upper() if config else ''
    plt.title(f'Range Extension: Direct vs Relayed — Protocol Comparison (Relays={relays})')
    plt.yscale('log')
    plt.legend(fontsize='small', ncol=2)
    plt.grid(True, which='both', ls='--', alpha=0.6)
    # Choose a sensible lower y-limit for log scale so small but nonzero finite rates remain visible
    try:
...
def create_optimal_hops_figure(x_data, y_data, total_dist, config, title=None):
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(x_data, y_data, 'bo-', linewidth=2)
    p_name = config.get('protocol', 'polar').upper()
    pa_name = config.get('pa_protocol', 'toeplitz').upper()
    if title is None:
        title = f'Optimal Hop Analysis (Server-Client BB84 {p_name}/{pa_name})\nTotal Dist={total_dist}km'
...
def create_range_extension_protocols_figure(distances, results, relays, config):
    ...
    for i, proto in enumerate(protocols):
        ...
        color = colors(i % 10)
        relayed_arr = np.asarray(relayed_rates, dtype=float)
        ax.plot(distances, relayed_arr, linestyle=linestyles['relayed'], color=color, linewidth=2, label=f'{proto} (relayed)')
        all_positive_vals.append(relayed_arr[relayed_arr > 0])
...
def build_multihop_serverclient_dashboard(
    ...
    sim_protocol = sim_config.get('protocol', 'polar')
    sim_pa = sim_config.get('pa_protocol', 'toeplitz')
    if selected_ec_protocols:
        sim_protocol = selected_ec_protocols[0]
    if selected_pa_protocols:
        sim_pa = selected_pa_protocols[0]

    hop_config = dict(sim_config)
    hop_config['protocol'] = sim_protocol
    hop_config['pa_protocol'] = sim_pa

    relay_counts, hop_rates = simulate_optimal_hops(
        total_distance=total_distance,
        relay_counts=relay_counts,
        num_qubits=num_qubits,
        link_capacity=link_capacity,
        sim_config=hop_config,
        chain_params=chain_params,
    )
    effective_rate = float(max(hop_rates)) if hop_rates else 0.0

    optimal_fig = create_optimal_hops_figure(relay_counts, hop_rates, total_distance, hop_config)
    plot_optimal_hops = _figure_to_base64(optimal_fig)
    plt.close(optimal_fig)

    range_protocols = selected_ec_protocols or [sim_protocol]
    range_pas = selected_pa_protocols or [sim_pa]
    ...
    range_fig = create_range_extension_protocols_figure(scan_distances, range_results, relays, hop_config)
    plot_range_extension = _figure_to_base64(range_fig)
    plt.close(range_fig)
...
def run_multihop_serverclient_example():
    ...
    # Optimal hops
    relays, rates = simulate_optimal_hops(total_distance=500.0, relay_counts=np.linspace(1, 30, 20, dtype=int).tolist(), num_qubits=10000, link_capacity=1e9, sim_config=SIM_CONFIG, chain_params=CHAIN_CONFIG)
    plot_optimal_hops(relays, rates, 500.0, SIM_CONFIG)

    # Range extension
    # Range extension: compare multiple EC protocols side-by-side
    protocols = ['polar', 'cascade', 'nr_ldpc_standard', 'ldpc_rateadaptive', 'winnow']
    dists, results = simulate_range_extension_protocols(protocols=protocols, max_distance=700, num_points=20, comparison_relays=3, num_qubits=10000, link_capacity=1e9, sim_config=SIM_CONFIG, chain_params=CHAIN_CONFIG)
